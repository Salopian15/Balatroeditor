...
from game_data import EDITIONS, EDITION_COLOURS
...
class ConsumableTab(ttk.Frame):
    ...
    def _build(self):
        ...
        bot = ttk.LabelFrame(self, text="Add Consumables", padding=10)
        bot.pack(fill="both", expand=True, padx=10, pady=(5, 10))

        self.picker_canvas = tk.Canvas(bot, bg="#16213e", highlightthickness=0)
        picker_sb = ttk.Scrollbar(bot, orient="vertical", command=self.picker_canvas.yview)
        ...
        self.picker_inner.bind("<Configure>", lambda e: self.picker_canvas.configure(scrollregion=self.picker_canvas.bbox("all")))
    ...
    def _populate_picker(self):
        ...
        # Tarots
        hdr = tk.Label(self.picker_inner, text="  Tarot Cards  ", bg="#8e44ad", fg="white", font=("Helvetica", 11, "bold"), padx=6, pady=2)
        hdr.grid(row=current_row, column=0, columnspan=6, sticky="w", pady=(8, 4), padx=4)
        current_row += 1
        col = 0
        for cid, cname in TAROTS:
            btn = tk.Button(self.picker_inner, text=cname, bg="#0f3460", fg="white", activebackground="#1a508b", activeforeground="white", relief="raised", bd=1, font=("Helvetica", 10), padx=8, pady=4, width=16, cursor="hand2", command=lambda c=cid: self._add_consumable(c))
        ...
        # Planets
        hdr = tk.Label(self.picker_inner, text="  Planet Cards  ", bg="#2980b9", fg="white", font=("Helvetica", 11, "bold"), padx=6, pady=2)
        hdr.grid(row=current_row, column=0, columnspan=6, sticky="w", pady=(8, 4), padx=4)
        current_row += 1
        col = 0
        for cid, cname in PLANETS:
            btn = tk.Button(self.picker_inner, text=cname, bg="#0f3460", fg="white", activebackground="#1a508b", activeforeground="white", relief="raised", bd=1, font=("Helvetica", 10), padx=8, pady=4, width=16, cursor="hand2", command=lambda c=cid: self._add_consumable(c))
            btn.grid(row=current_row, column=col, padx=3, pady=2, sticky="w")
            col += 1
            if col >= 4:
        ...
        # Spectrals
        hdr = tk.Label(self.picker_inner, text="  Spectral Cards  ", bg="#c0392b", fg="white", font=("Helvetica", 11, "bold"), padx=6, pady=2)
        hdr.grid(row=current_row, column=0, columnspan=6, sticky="w", pady=(8, 4), padx=4)
        current_row += 1
        col = 0
        for cid, cname in SPECTRALS:
            btn = tk.Button(self.picker_inner, text=cname, bg="#0f3460", fg="white", activebackground="#1a508b", activeforeground="white", relief="raised", bd=1, font=("Helvetica", 10), padx=8, pady=4, width=16, cursor="hand2", command=lambda c=cid: self._add_consumable(c))
            btn.grid(row=current_row, column=col, padx=3, pady=2, sticky="w")
            col += 1
            if col >= 4:
    ...
    def _refresh(self):
        ...
        for i, c in enumerate(cons):
            ...
            border_color = "#f39c12" if i in getattr(self, 'selected_indices', set()) else "#333"
            card = tk.Frame(self.inner, bg=border_color, padx=2, pady=2)        
            card.pack(side="left", padx=5, pady=5)

            bg_color = "#3d3d6e" if i in getattr(self, 'selected_indices', set()) else "#2d2d4e"
            inner_c = tk.Frame(card, bg=bg_color, width=80, height=110)
            inner_c.pack_propagate(False)
            inner_c.pack()

            display_name = center.replace("c_", "").capitalize()
            for lookup in (TAROTS + PLANETS + SPECTRALS):
                if lookup[0] == center:
                    display_name = lookup[1]
                    break

            lbl = tk.Label(inner_c, text=display_name, bg=bg_color, fg="white", font=("Helvetica", 10, "bold"), wraplength=70)
    ...
    def _remove_selected(self):
        if not self.data or not getattr(self, 'selected_indices', None): return 
