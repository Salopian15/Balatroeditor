...
from game_data import (
    ...
    edition_name_to_id, jokers_by_rarity,
)
...
# Rarity groupings derived from the rarity field on each joker — no hardcoded slices.
JOKER_GROUPS = jokers_by_rarity()
...
# Display order for rarity groups
...
class JokerTab(ttk.Frame):
    ...
    def _build(self):
        ...
        self.joker_canvas.create_window((0, 0), window=self.joker_inner, anchor="nw")
    ...
    def _populate_picker(self, filter_text=""):
        ...
        for group_name in RARITY_ORDER:
            jokers = JOKER_GROUPS.get(group_name, [])
    ...
    def _on_edition_change(self, event=None):
        ...
        ed_key = edition_name_to_id(self.edition_var.get())
    ...
    def _create_joker_card(self, idx, info, selected):
        ...
        clickables.append(id_lbl)

        # Click to select — bind all child widgets
        for widget in clickables:
            widget.bind("<Button-1>", lambda e, i=idx: self._select_joker(i, e))
